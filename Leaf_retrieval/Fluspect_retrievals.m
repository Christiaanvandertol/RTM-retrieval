...
load ../data/input/fluspect_data/Optipar2017_ProspectD.mat
%O = dlmread('../data/input/fluspect_data/Optipar_2013.csv',',',1,0);
%O = xlsread('../data/input/fluspect_data/Optipar_2015.xlsx');
%load('../data/input/fluspect_data/08-Nov-2016OptiparKcaVZ.mat');
% optipar.nr     = O(:,2);
% optipar.Kab    = O(:,3);
% optipar.Kca    = O(:,4);
% optipar.Ks     = O(:,5);
...
if length(c)>1
    [leafbio_all.Cab,leafbio_all.Cw,leafbio_all.Cdm, leafbio_all.Cs,leafbio_all.Cca,leafbio_all.N,leafbio_all.V2Z,leafbio_all.fqe_opt,leafopt_all.er2] = deal(zeros(length(c),1));
...
LB              = [0   .001   0    0   0 1    ]'; % lower boundaries
