...
target          = d(10,1); %#ok<*NASGU> %0: calibrate T&R, 1: calibrate only R; 2: calibrate only T

%% which spectral region should I calibrate?
wlmin           = d(11,1);          % starting wavelength (nm)
wlmax           = d(12,1);         % ending wavelength (nm)

%% initialize parameters for retrieval 
% these will be calibrated to your reflectance data if you said so above
leafbio.Cab     = d(13,1);           % chlorophyll content               [ug cm-2]
leafbio.Cdm     = d(14,1);        % dry matter content                [g cm-2]
leafbio.Cw      = d(15,1);        % leaf water thickness equivalent   [cm]
leafbio.Cs      = d(16,1);          % senescent material                [fraction]
leafbio.Cca     = d(17,1);            % carotenoids                       [?]
leafbio.N       = d(18,1);          % leaf structure parameter (affects the ratio of refl: transmittance) []
leafbio.fqe(2)  = d(19,1);                     % quantum yield
leafbio.fqe(1)  = d(20,1);
leafbio.V2Z     = 0;
